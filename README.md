...
The following diagram shows the database schema for the Tender Compliance Document Generator (TCDG) system, including tables, relationships, and key constraints.

![TCDG Database ERD](./docs/images/tcdg-erd.png "TCDG Database ERD")

### Table Overview

**TENANT** – Represents each tenant in the system  
- `id` (PK, uuid)  
- `name` (varchar)  
- `status` (varchar)  
...
**COMPANY** – Companies owned by tenants  
- `id` (PK, uuid)  
- `tenant_id` (FK, uuid)  
- `name`, `registration_number`, `bee_level`  
- `created_at` (timestamp)  

**DOCUMENT** – Documents uploaded/generated by companies/users  
- `id` (PK, uuid)  
- `tenant_id`, `company_id` (FK, uuid)  
- `type`, `status` (varchar)  
- `created_at` (timestamp)  

**TENDER** – Tender opportunities posted by tenants  
- `id` (PK, uuid)  
- `tenant_id` (FK, uuid)  
...
**COMPLIANCE_RESULT** – Evaluation of a company against a tender  
- `id` (PK, uuid)  
- `tenant_id`, `tender_id`, `company_id` (FK, uuid)  
- `score` (numeric)  
- `missing_items` (jsonb)  
- `created_at` (timestamp)  

**AI_JOB** – AI processing jobs for tenders  
- `id` (PK, uuid)  
- `tenant_id`, `tender_id` (FK, uuid)  
- `status`, `result_path` (varchar)  
- `created_at` (timestamp)  

**IDEMPOTENCY_KEY** – Used to prevent duplicate requests  
- `id` (PK, uuid)  
- `tenant_id` (FK, uuid)  
- `key`, `request_hash` (varchar)  
- `created_at` (timestamp)  

**AUDIT_LOG** – Tracks actions by users on entities  
- `id` (PK, uuid)  
- `tenant_id`, `user_id` (FK, uuid)  
- `action`, `entity_type` (varchar)  
- `entity_id` (uuid), `old_value`, `new_value` (jsonb)  
- `created_at` (timestamp)  

> *Figure: Database structure showing tables, primary keys, foreign keys, and relationships.*
